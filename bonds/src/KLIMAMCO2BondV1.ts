...
import { KLIMA_MCO2_BOND_V1, KLIMA_MCO2_BOND_V1_2, KLIMAMCO2_LPBOND_TOKEN, KLIMA_MCO2_PAIR, KLIMA_MCO2_BOND_V1_2_BLOCK} from '../../lib/utils/Constants'
...
export function handleDeposit(event: BondCreated): void {
    ...
    deposit.token = KLIMAMCO2_LPBOND_TOKEN;
    deposit.transaction = transaction.id
    deposit.bonder = bonder.id
    deposit.payout = toDecimal(event.params.payout, 9)

    if (event.block.number.lt(BigInt.fromString(KLIMA_MCO2_BOND_V1_2_BLOCK))) {
        deposit.daoIncome = getDaoIncome(KLIMA_MCO2_BOND_V1, deposit.payout)
    } else {
        deposit.daoIncome = getDaoIncome(KLIMA_MCO2_BOND_V1_2, deposit.payout)
    }
    ...
    deposit.carbonCustodied = getDiscountedPairCO2(event.params.deposit, KLIMA_MCO2_PAIR)
...
export function handleRedeem(event: BondRedeemed): void {
    ...
    redemption.token = KLIMAMCO2_LPBOND_TOKEN
...
export function handleBCV(event: ControlVariableAdjustment): void {
    let transaction = loadOrCreateTransaction(event.transaction, event.block)
    updateBondBCV(transaction.timestamp, KLIMAMCO2_LPBOND_TOKEN, event.params.newBCV)
}
