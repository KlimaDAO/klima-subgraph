import {
    KLIMA_USDC_PAIR, KLIMA_BCT_PAIR, NCT_USDC_PAIR,
...
let BIG_DECIMAL_1E3 = BigDecimal.fromString('1e3')
...
export function getKLIMAUSDRate(): BigDecimal {
    let pair = UniswapV2Pair.bind(Address.fromString(KLIMA_USDC_PAIR))

    let reserves = pair.getReserves()
    let reserve0 = reserves.value0.toBigDecimal()
    let reserve1 = reserves.value1.toBigDecimal()

    let klimaRate = reserve0.div(reserve1).times(BIG_DECIMAL_1E3)
